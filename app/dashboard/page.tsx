...
export default async function Dashboard() {
  // RBAC: redirects to /login if not authenticated
  ...
  return (
    <div className="shell py-10">
      ...
      <section className="card mt-7 p-6">
        <h2 className="text-xl font-black">My links</h2>
        <p className="mt-2 muted">
          Share product links in this format:{" "}
          <code>/go/product-id?ref={s.id}</code>
        </p>
        ...
        {clicks.length ? (
          ...
          <p className="mt-3 muted">No referral clicks yet.</p>
        )}
      </section>
    </div>
  );
